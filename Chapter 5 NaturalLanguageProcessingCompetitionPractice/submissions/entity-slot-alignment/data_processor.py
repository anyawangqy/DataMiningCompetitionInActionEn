...
class SCProcessor(DataProcessor):
        ...
        for turn in item["content"]:
            ...
            for ent in turn["info"]["ents"]:
                for position in ent["pos"]:
                    ...
                    ent_ids.add(ent["id"])
                    text1 = utterances[utterance_id][offset[0]:offset[1]].replace("_"," ")
    ...
    def get_instance(self, utterances, entities, ent_id, triple):
        if ent_id != "NA":
            ...
            for ent in entities_in_range:
                ...
                ent["type"] = "entity"
                text1 = text[ent["position"][0]:ent["position"][1]].replace("_"," ")
                text2 = ent["name"].replace("_"," ")
                if text1 != text2:
                    print(text1, text2)
                # assert text[ent["position"][0]:ent["position"][1]].replace("_"," ") == ent["name"].replace("_"," ")
            ...
            triple["type"] = "triple"
            text1 = text[triple["position"][0]:triple["position"][1]].replace("_"," ")
            text2 = triple["value"].replace("_"," ")
            if text1 != text2:
                print(247, text1, text2)
            # assert text[triple["position"][0]:triple["position"][1]].replace("_"," ") == triple["value"].replace("_"," ")
    ...
    def read_examples(self, input_file):
        ...
        #     exist_pos = []
        #     for turn in per_data['content']:
        #         if 'is_traing' not in turn:
        #             update_data.append(per_data)
        #             break
        #         else:
        #             exist_pos.append(turn['is_traing'])
        #     if len(exist_pos)!=0 and 'false' not in exist_pos:
        #         update_data.append(per_data)
        # for item in tqdm(update_data, desc="Reading from %s" % input_file):
