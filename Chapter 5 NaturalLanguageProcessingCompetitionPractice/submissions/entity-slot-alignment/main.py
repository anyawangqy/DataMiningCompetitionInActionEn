...
model_list = [
    'models/output_4fold/roformer_v2_chinese_char_base_new/checkpoint-40452/pytorch_model.bin',
    'models/output_5fold/roformer_v2_chinese_char_base_new/checkpoint-55440/pytorch_model.bin',
]


def inference_fn(trainer, dataloader):
    logits, _, _ = trainer.predict(
...
for model_path in model_list:
    ...
    test_dataloader = data_class(
        data_args, tokenizer, data_args.test_file, True
    )
    ...
    #     data_args, tokenizer, data_args.test_file, True
    # )
    # pred = inference_fn(model, test_dataloader)
    if final_pred is None:
        final_pred = pred
    else:
        for idx in range(len(pred)):
            final_pred[idx] += pred[idx]

final_pred = [i / len(model_list) for i in final_pred]

dump_result(data_args, final_pred, data_args.test_file)
