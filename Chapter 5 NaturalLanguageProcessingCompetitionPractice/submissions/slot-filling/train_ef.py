...
INPUT_DIR = '../input/'
OUTPUT_DIR = 'ef_roformer_add_dev/'
if not os.path.exists(OUTPUT_DIR):
    os.makedirs(OUTPUT_DIR)

roleid_path = INPUT_DIR +'role2idv1.json'
role2id = json.load(open(roleid_path))
role2id = {label: (id-1) for label, id in role2id.items() if label !="NA"}
id2role = {}
for k, v in role2id.items(): id2role[v] = k
print(f"num_lables = {len(role2id)}")
...
class CFG:
    ...
    print_freq=100
    num_workers=8
    model="/home/xmea2059/yk/SereTOD/pretrained/roformer_v2_chinese_char_base_new"
...
def get_score(pred_all, true_all,no_mark_num,epoch):
    ...
    for key, value in type_entity_posion.items():
        ...
        per_pred = len(R)
        per_true = len(T)
        per_f1, per_precision, per_recall = 2 * per_common / (per_pred + per_true+ (1e-8)), \
                                            per_common / (per_pred + (1e-8)), per_common / (per_true+ (1e-8))
        # LOGGER.info(id2type[key], "\tF1:{}\tP:{}\tR:{}\t".format(per_f1, per_precision, per_recall))
        LOGGER.info("{}\tF1:{:.3f},common:{},pred:{},true:{}".format(id2role[key],per_f1,per_common,per_pred,per_true))
...
if __name__ == '__main__':
    ...
    train = json.load(open(INPUT_DIR+'data_label_kfold_v1.1.json'))
    final_result = []
